...
export async function GET(request: Request) {
  ...
  try {
    ...
    const expectedState = cookieStore.get('oauth_youtube_state')?.value || '';
    if (!code || !state || state !== expectedState) {
      return NextResponse.redirect(`${appUrl}/accounts?error=YouTube OAuth state mismatch`);
    }
    ...
    const response = NextResponse.redirect(`${appUrl}/accounts?connected=youtube`);
    response.cookies.delete('oauth_youtube_state');
    return response;
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'YouTube OAuth failed';
