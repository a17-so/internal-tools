...
export default async function DashboardPage() {
  ...
  const cards = [
    { label: 'Connected Accounts', value: accountCount, icon: Layers },
    { label: 'Queued Jobs', value: queuedCount, icon: Clock3 },
    { label: 'Running Jobs', value: runningCount, icon: PlayCircle },
    { label: 'Failed Jobs', value: failedCount, icon: TriangleAlert },
  ];

  return (
    <div className="space-y-4">
      <section className="panel overflow-hidden p-5">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Publishing Control</p>
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-3xl font-semibold">Upload faster with fewer clicks</h2>
            <p className="mt-1 text-muted-foreground">Queue mixed media posts, route to any connected account, and dispatch with guardrails.</p>
          </div>
        ...
        {cards.map((card) => {
          ...
          return (
            <div key={card.label} className="panel p-4">
              <div className="inline-flex rounded-xl border bg-muted p-2 text-muted-foreground">
            ...
            <tbody>
              {recentJobs.map((job) => (
                <tr key={job.id} className="border-t text-sm">
                  ...
                  <td className="py-2 text-muted-foreground">{new Date(job.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
