...
def calculate_influencer_deal(avg_views, likes, comments, comment_quality, num_posts, base_cpm):
    ...
    # Step 8: Bonus Calculation (Scaling Based on Extra Views)
    if extra_views > 0:
        if extra_views <= min_views_for_profit * 0.25:
            bonus_amount = (extra_views / 100000) * 100  # Small bonus
        elif extra_views <= min_views_for_profit:
            bonus_amount = (extra_views / 100000) * 250  # Medium bonus
        else:
            bonus_amount = (min_views_for_profit / 100000) * 400  # Max cap on bonus
    else:
